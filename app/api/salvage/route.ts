...
async function serialize(i:any,userId:string){const allowed=i.posted_by===userId||i.claimed_by===userId;const db=admin();const {data:owner}=await db.from('users').select('name').eq('id',i.posted_by).single();const {address,posted_by,claimed_by,stripe_session_id,...rest}=i;let contact;if(allowed&&claimed_by){const {data}=await db.from('users').select('name,email,phone').eq('id',posted_by===userId?claimed_by:posted_by).single();contact=data;}return {...rest,lat:allowed?i.lat:Math.round(i.lat*100)/100,lng:allowed?i.lng:Math.round(i.lng*100)/100,owner_name:owner?.name,mine:posted_by===userId,claimedMine:claimed_by===userId,...(allowed?{address,contact}:{})};}
export async function GET(){try{const user=await getUser();if(!user)return fail('Please log in.',401);const id=user.userId,db=admin();const [p,l,n]=await Promise.all([db.from('users').select('*').eq('id',id).maybeSingle(),db.from('listings').select('*').or(`status.eq.available,posted_by.eq.${id},claimed_by.eq.${id}`).order('created_at',{ascending:false}).limit(500),db.from('notifications').select('*').eq('user_id',id).order('created_at',{ascending:false}).limit(50)]);if(p.error||l.error||n.error)return fail('The material exchange is temporarily unavailable.',503);const profileData=p.data?{...p.data,emailVerified:!!user.emailVerified}:null;return Response.json({profile:profileData,items:await Promise.all((l.data||[]).map(i=>serialize(i,id))),notifications:n.data});}catch{return fail('The material exchange is temporarily unavailable.',503);}}
export async function POST(req:Request){try{
 ...
 let coords;try{coords=x.locatedAddress===x.address&&hasCoordinates(x)?{lat:x.lat,lng:x.lng}:x.address===p.address&&hasCoordinates(p)?{lat:p.lat,lng:p.lng}:await geocode(x.address);}catch{return fail('We could not locate the pickup address. Use the location button to confirm your position, then publish.',422);}const id=crypto.randomUUID();const bytes=Buffer.from(x.photo.split(',')[1],'base64');if(bytes[0]!==255||bytes[1]!==216)return fail('Invalid photo.');const key=`${id}.jpg`;
  ...
  const {error}=await database.from('listings').insert({id,photo:`/api/photos/${id}`,category:x.category,material:x.material,condition:x.condition,title:x.title,description:x.description,address:x.address,...coords,price,posted_by:user.userId});
  if(error){
    console.error('Database insert listing error:', error);
    await database.storage.from('listing-photos').remove([key]);
    return fail(`Could not publish your listing: ${error.message}`,503);
  }
  return Response.json({id});
 }
 if(action==='claim'){
  if(p.role!=='buyer')return fail('A buyer account is required to claim materials.',403);
  if(!user.emailVerified)return fail('Please verify your email address before claiming materials.',403);
  const {data,error}=await database.rpc('claim_listing',{listing_id:body.id,buyer_id:user.userId});
  if(error)return fail('This item has already been claimed or is unavailable.',409);
  
  // Fetch seller info for free claim receipt
  const {data:seller}=await database.from('users').select('name,email,phone').eq('id',data.posted_by).maybeSingle();
  const serialized=await serialize(data,user.userId);
  const receipt={
    receiptNumber:`CLM-${data.id.slice(0,8).toUpperCase()}`,
    type:'claim',
    ...
    payment:{
      amount:0,
      currency:'usd',
      method:'Free Salvage Claim',
      status:'Confirmed'
    }
  };

  return Response.json({item:serialized,receipt});
  }
 if(action==='donate'){
