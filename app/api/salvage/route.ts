...
async function serialize(i:any,userId:string){const allowed=i.posted_by===userId||i.claimed_by===userId;const db=admin();const {data:owner}=await db.from('users').select('name').eq('id',i.posted_by).single();const {address,posted_by,claimed_by,...rest}=i;let contact;if(allowed&&claimed_by){const {data}=await db.from('users').select('name,email,phone').eq('id',posted_by===userId?claimed_by:posted_by).single();contact=data;}return {...rest,lat:allowed?i.lat:Math.round(i.lat*100)/100,lng:allowed?i.lng:Math.round(i.lng*100)/100,owner_name:owner?.name,mine:posted_by===userId,claimedMine:claimed_by===userId,...(allowed?{address,contact}:{})};}
