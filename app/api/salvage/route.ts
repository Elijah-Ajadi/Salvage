...
export async function GET(){
  try{
    ...
    const [p,l,n]=await Promise.all([
      ...
      db.from('listings').select('*').or(`status.eq.available,posted_by.eq.${id},claimed_by.eq.${id}`).order('created_at',{ascending:false}).limit(500),
    ...
    return Response.json({
      profile:profileData,
      items:await Promise.all((l.data||[]).map(i=>serialize(i,id))),
